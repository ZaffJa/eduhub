...
<div class="form-group">
    {{ Form::label('school location', 'Lokasi Sekolah') }}

    </br>
    Cari lokasi sekolah menggunakan kotak teks di bawah atau klik peta.
    {{ $location->latitude or null }}
    @if(old('lat') != null)
        <input name="lat" type="hidden" id="lat" value="{{ old('lat') }}">
        <input name="lng" type="hidden" id="lng" value="{{ old('lng') }}">
    @else
    ...
    @endif
    <input id="pac-input" class="controls" type="text" placeholder="Kotak Teks Carian">
...
<script>
    ...
    function initMap() {
        ...
        var pastLocation = null;

        @if(old('lat') != null)

                pastLocation = new google.maps.LatLng(
                parseFloat("{{ old('lat') }}").toFixed(6),
                parseFloat("{{ old('lng') }}").toFixed(6)
        );
        @else
                pastLocation = new google.maps.LatLng(
