                            ...
                            @else
                                @foreach($facility_img as $faci_img)
                                    @if( $faci_img->facility_id == $facility->id)
                                        <td>
                                            <a href="#imgBox" class="clickImg"
                                               src="{{$faci_img->path}}">{{$faci_img->filename}}</a>
                                        </td>
                                    @endif
                                @endforeach
                            @endif
                            <td><a href="{!! action('FacilityController@edit',array($typeid, $facility->id)) !!}">
                                    <button class="btn btn-info">Edit</button>
                                </a>
                                <button value="{!! route('faci.delete', [$typeid, $facility->id]) !!}"
